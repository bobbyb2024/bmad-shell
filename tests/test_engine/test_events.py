...
from bmad_orch.engine import (
    BaseEvent, StepStarted, StepCompleted, CycleStarted, CycleCompleted,
    EscalationChanged, LogEntry, ProviderOutput, RunCompleted,
    ErrorOccurred, ResourceThresholdBreached, EscalationLevel, LogLevel,
)
...
def test_escalation_changed_with_optional_none():
    ...
    event = EscalationChanged(previous_level=None, new_level=EscalationLevel.ATTENTION)
...
def test_escalation_changed_with_both_levels():
    event = EscalationChanged(
        previous_level=EscalationLevel.ATTENTION, new_level=EscalationLevel.ACTION
    )
...
def test_all_event_types_instantiate():
    ...
    events = [
        ...
        EscalationChanged(previous_level=None, new_level=EscalationLevel.OK),
        LogEntry(level=LogLevel.INFO, message="m", source="s"),
        ProviderOutput(provider_name="p", content="c", is_partial=False),
        RunCompleted(success=True, total_cycles=1, error_count=0),
