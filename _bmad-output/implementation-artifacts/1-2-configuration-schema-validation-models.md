...
- [ ] Task 1: Define new enum types (AC: 1, 3)
  - [ ] Add `Timing` enum (`step`, `cycle`, `end`) to `src/bmad_orch/types/__init__.py` — single enum for both commit and push timing (identical values, no justification for separate types)
  - [ ] `StepType` already exists — DO NOT recreate
- [ ] Task 2: Create Pydantic models in `src/bmad_orch/config/schema.py` (AC: 1, 4, 5)
  ...
  - [ ] `CycleConfig(BaseModel)`: `steps: list[StepConfig]` (min_length=1), `repeat: int = 1` (≥1), `pause_between_steps: float | None = None` (≥0 if set), `pause_between_cycles: float | None = None` (≥0 if set)
  - [ ] `GitConfig(BaseModel)`: `commit_at: Timing`, `push_at: Timing`
  - [ ] `PauseConfig(BaseModel)`: `between_steps: float` (≥0), `between_cycles: float` (≥0), `between_workflows: float` (≥0)
  - [ ] `ErrorConfig(BaseModel)`: `retry_transient: bool = True`, `max_retries: int = 3` (≥1), `retry_delay: float = 10.0` (≥0)
...
- [ ] Task 3: Add cross-field validation (AC: 1, 2, 3, 6)
  ...
  - [ ] `@model_validator` on `OrchestratorConfig`: enforce `providers` has ≥1 entry and `cycles` has ≥1 entry
  ...
  - [ ] Wrap Pydantic `ValidationError` in `ConfigError` with clear, actionable messages — must preserve field name, invalid value, and valid options (for enums) in the wrapped message
